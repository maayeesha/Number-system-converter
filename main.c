...
void userInput(
    int); // takes the user input and validates for further opertaions
...
void userInput(int choice) {
  ...
  } else if (choice == 4) // HexaDecimal input validation code
  {
    ...
    for (j = 0; j < i; j++) {
      if ((hexa[j] >= 'A' && hexa[j] <= 'F') ||
          (hexa[j] >= 'a' && hexa[j] <= 'f') || isdigit(hexa[j]))
        k++;
      else {
        flag = 1;
        break;
    ...
    if (flag == 1) {
      printf("\n\nError: Hexadecimal digits can only be between 0 to 9 & A to "
             "F. \n");
      printf("Press any key to continue... \n");
      getch();
      welcomeScreen();
